...
document.getElementById('nav-parents').addEventListener('click', () => switchView('parents'));
...
// Global flag to track if parent is authorized in this session
...
function updateNavHighlights(view) {
  ...
  if (view === 'parents') {
    ...
    const bottomParents = document.querySelector('.bottom-nav-item[data-view="parents"]');
    if (bottomParents) bottomParents.classList.add('active');
  }
}
...
function isLevelDisabledByParent(level) {
...
let activeParentsTab = 'progress';

// secure 4-digit PIN gateway
function promptParentPin(onSuccess, onCancel) {
...
// Render Parent Portal main view
...
// 1. Progress Tab
function renderParentsProgressTab() {
...
function findWordEmoji(wordStr) {
...
// 2. Controls Tab
function renderParentsControlsTab() {
  const content = document.getElementById('parents-tab-content');
  if (!content) return;
...
// 3. Profiles Tab
function renderParentsProfilesTab() {
  const content = document.getElementById('parents-tab-content');
  if (!content) return;
...
// 4. Export Tab
function renderParentsExportTab() {
  const content = document.getElementById('parents-tab-content');
  if (!content) return;
...
// Sync current state into the profiles array
...
// Load chosen profile's states into active states
...
// 4. Profile Selection Overlay
function renderProfileSelectionOverlay() {
...
// 5. Screen time lockout trigger
let isLockedByTimeLimit = false;

function triggerTimeLimitLock() {
